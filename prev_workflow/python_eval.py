...
def report(fish_list, result_list, name):
    ...
    print(name , "TN:",tn, "FP:",fp, "FN:",fn, "TP:",tp, "Sensitivity:",sensitivity, "Specificity:",specificity, "AUC:",auc)
...
prob_aucs = [roc_auc_score(train_label,[ 1 if x >= perc_prob[i] else 0 for x in train_softmax ]) for i in range(len(perc_prob))]
