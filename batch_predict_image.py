...
t_image_label_ds = tf.data.Dataset.zip((t_image_ds1,t_image_ds2,t_label_ds,t_label_ds_ori,t_path_ds1,t_path_ds2,t_category )) 
...
with strategy.scope():
    for step, (image1,image2,label,label_ori,file1,file2,cate) in enumerate(train_ds):
        ...
        for i in range(0,len(lst_label)):
            ...
            elem=lst_file1[i].decode("utf-8")+' '+str(lst_label_ori[i][0])+' '+lst_cate[i].decode("utf-8")
